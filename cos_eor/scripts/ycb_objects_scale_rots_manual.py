...
def adjust_assets(scene):
    sim = None
    # adjust assets on the sofa within "Rs_int"
    sim = get_sim(scene, sim, "cos_eor/configs/dataset/build_dataset.yaml")
    meta_keys, object_ids, metadata, metadata_dir = sim.init_metadata_objects()
    debug_sim_viewer(sim, False, meta_keys, object_ids, metadata, metadata_dir)

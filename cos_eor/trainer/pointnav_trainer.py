...
class CosRearrangementTrainer(BaseRLTrainer):
    ...
    def _setup_explore_module(self):
        self.explore_module = ExploreModule(self.config.RL.POLICY.explore)
    ...
    def _setup_policy(self, ckpt_dict):
        ...
        self.policy = HiePolicy(nav_param, self.rank_module,
                                self.explore_module, self.config.RL.POLICY, self.config.BASE_TASK_CONFIG.TASK, debug_params)
    ...
    def _collect_rollout_step(
        ...
        with torch.no_grad():
            ...
            # get either pointgoals or actions here
        ...
        print(f"Act time: {pth_time} || Step time: {env_time}")
...
def run_exp(exp_config: str, run_type: str, ckpt_path, opts=None, **kwargs):
    ...
    if kwargs['debug'] and "RGB_SENSOR_3RD_PERSON" not in config.SENSORS and "cos-hab2" in os.getcwd():
        # add 3rd person semantic sensor

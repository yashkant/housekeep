...
sys.path.append('/srv/rail-lab/flash5/kvr6/dev/housekeep_csr/CSR')
...
class GeneratePreferenceDataset():
    ...
    def __init__(self, 
        ...
        self.ep_r_nums = [f.replace('.json','').replace('obs_','') for f in files_list]


        self.get_resnet = lambda episode_run_num, obj_1, obj_2 : torch.load(open(os.path.join(root_dir, 
                                        'ihlen_1_int', 
                                        'baseline_phasic_oracle',
                                        'resnet',
                                        '{}_{}_{}.pt'.format(obj_1, obj_2, episode_run_num)
                                        )))
        
        self.get_image = lambda episode_run_num : torch.load(open(os.path.join(root_dir, 
                                        'ihlen_1_int', 
                                        'baseline_phasic_oracle',
                                        'images',
    ...
    def get_features(self, episode_run_num, object_idx):
        ...
        bb_data_path = os.path.join(RAW_DATA_PATH,
                                    f'csr/XYZ_{episode_run_num}.json')

        with open(bb_data_path, 'r') as fh:
            crop_data_dict = json.load(fh)
            bb = [item for item in crop_data_dict['items'] 
            ...
            if self.use_episode(episode):
                if self.use_obj(o1) and self.use_obj(o2):
                    if self.is_obj(o1) and not self.is_obj(o2):
                        ...
                        csr_feature1, clip_feature1 = self.get_features(file, o1)
                        csr_feature2, clip_feature2 = self.get_features(file, o2)

                        room_o2 = OBJECT_KEY_FILTER[o2].split('|')[0]
